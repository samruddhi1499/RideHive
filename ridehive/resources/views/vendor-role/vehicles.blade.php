...
<div class="bg-white p-6 rounded-lg shadow mb-8">
    ...
    <form action="{{ route('vendor-role.vehicles.store') }}" method="POST" class="space-y-4" enctype="multipart/form-data">
        @csrf
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                <label for="model" class="block text-gray-600">Model</label>
                <input type="text" id="model" name="model" class="border rounded w-full py-2 px-4 focus:ring-[#e75480]" placeholder="Enter vehicle model" required>
            </div>
            <div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="number" id="price_per_day" name="price_per_day" class="border rounded w-full py-2 px-4 focus:ring-[#e75480]" placeholder="Enter price per day" step="0.01" required>
            </div>
            <div>
        ...
        <div>
            ...
            <input type="file" id="image" name="image" class="border rounded w-full py-2 px-4 focus:ring-[#e75480]" accept="image/*" required>
        </div>
        ...
        <tbody>
            @forelse ($vehicles as $vehicle)
                <tr class="border-t">
                    ...
                    <td class="py-2 px-4">
    @if ($vehicle->image)
        <img src="data:image/jpeg;base64,{{ base64_encode($vehicle->image) }}" alt="{{ $vehicle->model }}" class="w-50 h-20 object-cover rounded">
    @else
        <span>No Image</span>
    @endif
</td>
                    <td class="py-2 px-4 flex gap-2">
                        <a href="{{ route('vendor-role.vehicles.edit', $vehicle) }}" class="text-[#e75480] border border-[#e75480] rounded px-4 py-1 hover:bg-[#e75480] hover:text-white">Edit</a>
                        <form action="{{ route('vendor-role.vehicles.destroy', $vehicle) }}" method="POST">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="text-red-600 border border-red-600 rounded px-4 py-1 hover:bg-red-600 hover:text-white">Delete</button>
                        </form>
                    </td>
                </tr>
            @empty
